...
import { getAll, getNames, postAdd, postBulk, deleteData, updateData } from "../../../../apiQuery/dosenPembimbing.query";
...
// import { bulkDataDosen, deleteSingleDosen, updateSingleDosen } from '../../reduxRtk/formOption/formOptionSlice';
// import { useDispatch, useSelector } from 'react-redux';
// import { getDataDosen, resetDosen, addSingleDosen} from "../../reduxRtk/formOption/formOptionSlice"
...
const DosenPembimbing = () => {
    ...
    const { status, data } = useQuery(['dosenPembimbing'], () => getAll(user?.accessToken))
    ...
    const addSingleDosen = useMutation(postAdd, {
        onSuccess: () => {
            queryClient.invalidateQueries('dosenPembimbing')
        }
    })
    ...
    // useEffect(() => {
    //     if (formOptionUpdateDosenSuccess) {
    //         toast('Update berhasil disimpan.', { position: "top-right", hideProgressBar: false, closeOnClick: false, className: 'bg-success text-white' })

    //         // habis updateSingleDosen, fetch data from server biar cepet
    //         if (isEdit) {
    //             dispatch(getDataDosen())
    //         }

    //         dispatch(resetDosen())
    //     }
    // }, [formOptionUpdateDosenSuccess])

    // useEffect(() => {
    //     if (formOptionUpdateDosenError) {
    //         toast(formOptionMessage, { position: "top-right", hideProgressBar: false, closeOnClick: false, className: 'bg-danger text-white' })
    //         // setListDataDosen(dataDosen)
    //         dispatch(resetDosen())
    //     }
    // }, [formOptionUpdateDosenError])
    ...
    const columns = memoHook(
        () => [
            ...
            },
            {
                ...
                //   onClick={() => {
                //     const dataPermohonan = cellProps.row.original;
                //     handlePermohonanClick2(dataPermohonan);
                //   }}
                >
    ...
    document.title = "Dosen Pembimbing | Velzon - React Admin & Dosen Pembimbing Template";
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Dosen Pembimbing" pageTitle="Dosen Pembimbings" />
